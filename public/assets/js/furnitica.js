...
Line 1831:2 Transforming const to the configured target environment ("es5") is not supported yet
Line 1834:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 1835:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 1836:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 1839:6 Transforming const to the configured target environment ("es5") is not supported yet
Line 1846:6 Transforming const to the configured target environment ("es5") is not supported yet
Line 1860:10 Transforming const to the configured target environment ("es5") is not supported yet
Line 1861:10 Transforming const to the configured target environment ("es5") is not supported yet
Line 1862:10 Transforming const to the configured target environment ("es5") is not supported yet
Line 1863:10 Transforming const to the configured target environment ("es5") is not supported yet
                        ...
                        if (optionValues.indexOf(value) < 0) {
                            ...
                            if (is_color) {
                                if (variant.featured_image){
                                  options += '<label for="' + forText + '" style="background-image: url(' + variant.featured_image.src + '.png)"><img class="crossed-out" src="' + assetUrl +'" /></label>';
                                } else {
...
$(function() {
  const cartQtyWrappers = $('.cart_qty_wrapper');

  cartQtyWrappers.each(function (params) {
    const decreseBtn = $(this).find('.dec');
    const increseBtn = $(this).find('.inc');
    const input = $(this).find('input.qty');

    decreseBtn.on('click', function () {
...
$(function() {
  window.searchHistory = {
    ...
    eventBinds: function() {
      this.searchForm.each(function () {
        $(this).on("submit", function (e) {
          e.preventDefault();
          const searchField = $(this).find("[name=q]");
          const searchTerm = searchField.val();
          const savedTerms = JSON.parse(window.localStorage.getItem("yellow_search_terms"));
          const history =  savedTerms ? savedTerms : [];
          history.unshift(searchTerm);
          if (history.length > 10) history.length = 10;
          window.localStorage.setItem("yellow_search_terms", JSON.stringify(history));
          window.location.href = `/search?type=product&q=${searchTerm}`;
        });
      })

      const self = this;
      this.searchForm.each(function() {
        $(this).find("[name=q]").on("keyup focus", function (e) {
          self.searchForm.find(".search_history").toggleClass("active", !$(this).val());
        });
      });

      $(document).on("click", function (e) {
          // console.log("here 1")
          if (e.currentTarget != self.searchForm.eq(0)) {
            self.searchForm.find(".search_history").removeClass("active");
            // console.log("here")
          }
      })
    ...
    remove: function (term) {
      const searchTerms = JSON.parse(window.localStorage.getItem("yellow_search_terms"));
      const newSearchTerms = searchTerms.filter(st => st !== term);
      window.localStorage.setItem("yellow_search_terms", JSON.stringify(newSearchTerms));
      this.render();
      this.searchForm.each(function () {
        if ($(this).find("[name=q]").length > 0) $(this).find("[name=q]").addClass("active");
      })
    },
    render() {
      ...
      if (!searchTerms || !searchTerms.length) {
        this.searchForm.each(function () {
          $(this).find(".search_history").remove();
        });
        return;
      }
      
      const termsListHtmls = searchTerms.map(st => `<li><a href="/search?type=product&q=${st}">${st}</a><span onclick="window.searchHistory.remove('${st}')">X</span></li>`).join("");
      const htmls = `
        <div class="search_history">
          <h4 class="search_history-title">Your recent searches</h4>
          <ul>${termsListHtmls}</ul>
        </div>
      `;
      this.searchForm.each(function () {
        $(this).find(".search_history").remove();
